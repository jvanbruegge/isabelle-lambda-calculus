...
next
  ...
  proof (induction vals arbitrary: \<tau>1 \<tau>2 rule: term_elist.inducts(2)[of "\<lambda>x. True"])
    ...
    next
      case (TCons x1 x2)
      from TCons(4) obtain \<sigma>1 a1 k1 where P1: "\<Gamma> , \<Delta> \<turnstile> Ctor D x2 ENil : \<forall> a1:k1. \<sigma>1" "\<Gamma> , \<Delta> \<turnstile>\<^sub>t\<^sub>y x1 : k1" "\<tau>1 = \<sigma>1[x1/a1]" by blast
      from TCons(5) obtain \<sigma>2 a2 k2 where P2: "\<Gamma> , \<Delta> \<turnstile> Ctor D x2 ENil : \<forall> a2:k2. \<sigma>2" "\<Gamma> , \<Delta> \<turnstile>\<^sub>t\<^sub>y x1 : k2" "\<tau>2 = \<sigma>2[x1/a2]" by blast
